<!doctype html>
<html class="no-js" lang="">
    <head>
        <meta charset="utf-8">
        <meta http-equiv="x-ua-compatible" content="ie=edge">
        <title>Big Games</title>
        <meta name="description" content="">
        <meta name="viewport" content="width=device-width, initial-scale=1">

        <link rel="manifest" href="site.webmanifest">
        <link rel="apple-touch-icon" href="icon.png">
        <!-- Place favicon.ico in the root directory -->

        <link rel="stylesheet" href="bootstrap/dist/css/bootstrap.min.css">
        <link rel="stylesheet" href="static/css/style.css">

        <!-- Font-Awesome -->
        <link rel="stylesheet" type="text/css" href="font-awesome/css/font-awesome.min.css">
        <link rel="font" type="text/css" href="font-awesome/fonts/fontawesome-webfont.woff">

        <!-- Owl Carousel -->
        <link rel="stylesheet" type="text/css" href="static/css/owl.carousel.min.css">
	    <link rel="stylesheet" type="text/css" href="static/css/owl.theme.default.min.css">

	    <meta name="viewport" content="width=device-width, initial-scale=1">

    </head>
    <body>
        <!--[if lte IE 9]>
            <p class="browserupgrade">You are using an <strong>outdated</strong> browser. Please <a href="https://browsehappy.com/">upgrade your browser</a> to improve your experience and security.</p>
        <![endif]-->

       <!-- Start -->
       	<section class="root">
       		<nav class="navbar navbar-default other-nav">
	            <div class="container-fluid">
		            <div class="row">
		            	
		                <div class="navbar-header">
		                    <button type="button" class="navbar-toggle collapsed" data-toggle="collapse" data-target="#navbar" aria-expanded="false" aria-controls="navbar">
						        <span class="sr-only">Toggle navigation</span>
						        <span class="icon-bar"></span>
						        <span class="icon-bar"></span>
						        <span class="icon-bar"></span>
						    </button>
		                </div>
		                <div id="navbar" class="navbar-collapse collapse no-padding">
		                    <div class="col-md-2 small-logo">
		                        <a href="index.html"><img src="static/img/bigg.png" class="mlogo" /></a>
		                    </div>
		                    <div class="col-md-4"></div>
		                    <div class="pull-right scart">
		                        <ul class="nav navbar-nav navbar-adjust top-nav">
		                            <!-- <li class="active"><a href="#/">Home<span class="sr-only">(current)</span></a></li> -->
		                            <li class="stake-cart"><a href="#">
		                            	<img src="static/img/red-shopping-cart.png" alt="">
		                            	STAKE
		                        	</a></li>
		                            <li class="reg-top-btn"><a href="#">Register</a></li>
		                            <li class="login-top-btn"><a href="#">Log In</a></li>
		                        </ul>
		                    </div>

		                    <div class="col-md-12 nav-below">
			                	<ul class="nav navbar-nav navbar-adjust">
			                	    <li><a href="#">Home</a></li>
			                	    <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">All Lotteries <span class="caret"></span></a>
			                	    	<ul class="dropdown-menu dropdown-below">
			                	    	    <li><a href="#">lotteries</a></li>
			                	    	    <li><a href="#">All</a></li>
			                	    	    <li><a href="#">Wind</a></li>
			                	    	</ul>
			                	    </li>
			                	    <li><a href="#">Promotions</a></li>
			                	    <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">Result <span class="caret"></span></a>
			                	    	<ul class="dropdown-menu dropdown-below">
			                	    	    <li><a href="#">Promotions</a></li>
			                	    	    <li><a href="#">Promotions</a></li>
			                	    	    <li><a href="#">Promotions</a></li>
			                	    	</ul>
			                	    </li>
			                	</ul>
			                </div>
		                </div>
		                <!--/.nav-collapse -->
		            </div>
	            </div>
	            <!--/.container-fluid -->
	        </nav>

	        <section class="logo-burger">
	        	<div>
	        		<a href="index.html"><img src="static/img/bigg.png" class="mlogo" /></a>
	        	</div>
	        	<div>
	        		<p>&#9776;</p>
	        	</div>
	        </section>

	        <section class="sidemenu">
	        	<div class="close-menu">
	        		<p>X</p>
	        	</div>
	        	 <div class="menu-inside">
                    <ul class="nav navbar-nav navbar-adjust right-link">
                        <!-- <li class="active"><a href="#/">Home<span class="sr-only">(current)</span></a></li> -->
                        <li><a href="#">Home</a></li>
                        <li class="dropdown"><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false">All Lotteries <span class="caret"></span></a>
                        	<ul class="dropdown-menu">
                        	    <li></li>
                        	    <li></li>
                        	    <li></li>
                        	    <li></li>
                        	</ul>
                        </li>
                        <li><a href="#">Promotions</a></li>
                        <li class="top-reg-btn"><a href="#">Register</a></li>
                        <li class="top-login-btn"><a href="#">Log In</a></li>
                        <li><a href="#"><img src="static/img/red-shopping-cart.png" alt=""></a></li>
                        <li><a href="#" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-haspopup="true" aria-expanded="false"><img src="static/img/nigeria.png" alt=""> <span class="caret"></span></a>
                        	<ul class="dropdown-menu">
                        	    <li></li>
                        	    <li></li>
                        	    <li></li>
                        	    <li></li>
                        	</ul>
                        </li>
                    </ul>
                </div>
	        </section>

	        <section class="account-banner">
	        	<div class="container">
	        		<div class="row">
	        		</div>
	        	</div>
	        </section>

	        <section class="five-ninety-card">
	        	<div class="container-fluid">
	        		<div class="row">
	        			<div class="col-md-12 no-padding">
	        				<div class="play-card">
	        					<div class="top-area">
	        						<div class="the-logo">
	        							<img src="static/img/fivelogo.png" alt="">
	        						</div>
	        						<div class="next-draw">
	        							<p>Next Draw In</p>
	        							<p><i class="fa fa-clock-o"></i> 00:56:03</p>
	        						</div>
	        						<div class="to-win">
	        							<p>To Win</p>
	        							<h2>&#8358;500,000</h2>
	        						</div>
	        					</div>
	        					<div class="select-stake">
	        						<?php require_once ('connections/biggames.php')?>

<?php
 session_start();
if (isset($_SESSION['userSession'])!="") {
 header("Location: five.html");
}

$inactive = 600; // Set timeout period in seconds

if (isset($_SESSION['timeout'])) {
    $session_life = time() - $_SESSION['timeout'];
    if ($session_life > $inactive) {
        session_destroy();
        header("Location: index.php");
    }
}
$_SESSION['timeout'] = time();


$conn    = Connect();
$full_name =     $conn->real_escape_string($_POST['full_name']);
$reg_email    = $conn->real_escape_string($_POST['reg_email']);
$reg_name   = $conn->real_escape_string($_POST['reg_name']);
$reg_id   = $conn->real_escape_string($_POST['reg_id']);
$reg_pass1    = $conn->real_escape_string($_POST['reg_pass1']);
$reg_pass2 = $conn->real_escape_string($_POST['reg_pass2']);
$reg_phone = $conn->real_escape_string($_POST['reg_phone']);
$reg_date = date("Y-m-d h:i:s");
$com_code = md5(uniqid(rand()));


//declare two session variables and assign them
    
 if (!$reg_email){ 
echo("<font color=red><strong>Error:</strong>Please enter a valid Email Address</font>". '<br>' . '<a href="login.php">Back </a>');
exit;}


$EMAILValidation= '/^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,4}$/';
  if(!preg_match($EMAILValidation, $reg_email)) {
  echo( "The Email you entered does not appear to be valid.<br/>" . '<a href="javascript:history.go(-1);">Back </a>'); 
 exit;}

   if (!$reg_id){ 
echo("<font color=red><strong>Error:</strong>Please enter your Username</font>". '<br>' . '<a href="login.php">Back </a>');
exit;}

if (!$reg_phone){ 
echo("<font color=red><strong>Error:</strong>Please enter your Mobile Number</font>". '<br>' . '<a href="login.php">Back </a>');
exit;}

   if (!$reg_pass1){ 
echo("<font color=red><strong>Error:</strong>Please enter your password</font>". '<br>' . '<a href="login.php">Back </a>');
exit;}

$PasswordValidation='/^.*(?=.{8,})(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).*$/';

 if(!preg_match($PasswordValidation, $reg_pass1)) {
 echo("<p>Password must be at least 8 characters/ and must contain at least one lower case letter, one upper case letter and one digit.</p><br/>".
   '<a href="javascript:history.go(-1);">Back </a>');
exit;}
   
      if ($reg_pass2 != $reg_pass1){ 
echo("<font color=red><strong>Error:</strong> Your Password does not match</font>". '<br>' . '<a href="javascript:history.go(-1);">Back </a>');
exit;}

$num   = '50f90';
$id = rand(0000,9999);

/* CREATE YOUR MYSQL CONNECTION */
$user_list = mysql_query("SELECT * FROM bigreg");
while ($user = mysql_fetch_array($user_list))
{
    if ($id == $user['id'])
    {
        echo('Already exist.');
    }
    else
    {
        /* YOUR CODE */
    }
}

$reg_user = $num.$id;

// check for duplicate STATUS
$checkDuplicate = "SELECT reg_email FROM bigreg
WHERE reg_email = '$reg_email'";

$result = $conn->query($checkDuplicate) or die(mysql_error());
$numRows = mysqli_num_rows($result);
  
// if $numRows is positive, the STATUS is already in use
if ($numRows) {
echo '<font color="red">' ."$reg_email You have already registered. Please Login to play games or sign up with a different email address." .'</font>'.'<a href="login.php">LOGIN </a>';
}

 
// otherwise, it's OK to insert the details in the database
else {
	
	//process the mail
$to1 = $reg_email;
$to2 = "info@superiorgamesng.com";

 $subject1 = "Thank You! Registration Received!";
 $subject2 = "New Registration";
 $body1 = "This is to notify you that we have received the Registration submitted via www.superiorgamesng.com Kindly activate you account by clicking on the link below. \n http://www.superiorgamesng.com/BigBet/biggames/verify.php?passkey=$com_code \n http://www.superiorgamesng.com/privatenew/doc/games_rules.pdf \n\n Here are your  LOGIN details below\n\n LOGIN DETAILS  \n\n" . "Email Address:" . $reg_email . "\n\n". "Username:" . $reg_user ; 
 
 $body2 = "This is to notify you that a new member has register via www.superiorgamesng.com \nThe registration details are listed below: \n\n". "Email:" . $reg_email . "\n\n". "Username:" . $reg_user;
 
 $headers = 'From: info@superiorgamesng.com' . "\r\n" .
    'Reply-To: info@superiorgamesng.com' . "\r\n" .
    'X-Mailer: PHP/' . phpversion();

 
    
    mail($to1, $subject1, $body1, $headers );
	mail($to2, $subject2, $body2, $headers );
	;

	 
$query   = "INSERT into bigreg (reg_email,reg_user,reg_pass,reg_phone, reg_id, reg_date, com_code) VALUES('" . $reg_email . "', '" . $reg_user . "','".md5($reg_pass1)."','" . $reg_phone . "','" . $reg_id . "', '" . $reg_date . "', '" . $com_code . "')";
$success = $conn->query($query);

 
if (!$success) {
    die("Couldn't enter data: ".$conn->error);
 
}

echo 'Thank You for taking your time to register, Proceed to <a href="login.php">LOGIN </a> <br>';
}
$conn->close();

 
?>
	        <footer class="section-area">
	        	<div class="telco-reach">
		        	<div class="pay-telco">
		        		<img src="static/img/mastercard.png" alt="">
		        		<img src="static/img/visa.png" alt="">
		        		<img src="static/img/logo.png" alt="">
		        		<img src="static/img/glo-logo.png" alt="">
		        	</div>
		        	<div class="reach-us">
		        		<p>You Can Reach Us Via:</p>
						<span>
			        		<a href=""><i class="fa fa-phone"></i></a>
			        		<a href=""><i class="fa fa-envelope"></i></a>
			        		<a href=""><i class="fa fa-facebook-square" aria-hidden="true"></i></a>
			        		<a href="" class="link-circle"><i class="fa fa-twitter circle-social" aria-hidden="true"></i></a>
			        		<a href="" class="link-circle"><i class="fa fa-instagram circle-social" aria-hidden="true"></i></a>
						</span>
		        	</div>
	        	</div>
	        	<div class="name-link">
	        		<p>BIG GAMES</p>
	        		<ul>
	        		    <li>About Us</li>
	        		    <li>Terms of Use</li>
	        		    <li>Privacy Policy</li>
	        		    <li>Site Maps</li>
	        		    <li>Tell A Friend</li>
	        		    <li>RSS</li>
	        		</ul>
	        	</div>
	        	<div class="copy-disclaimer">
	        		<p>&copy; SuperiorGames 2017</p>
	        		<p><i class="footer-age">18</i> You must be 18years old and above to play or claim a prize</p>
	        	</div>
	        </footer>
       	</section>
        <script src="jquery/dist/jquery.min.js"></script>
        <script src="bootstrap/dist/js/bootstrap.min.js" type="text/javascript" charset="utf-8" async defer></script>
        <script>window.jQuery || document.write('<script src="js/vendor/jquery-3.2.1.min.js"><\/script>')</script>
        <script type="text/javascript" src="js/owl.carousel.min.js"></script>

        <script>
	        $('.owl-carousel').owlCarousel({
	            loop:true,
	            margin:10,
	            nav:true,
	            responsive:{
	                0:{
	                    items:1
	                },
	                600:{
	                    items:3
	                },
	                1000:{
	                    items:6
	                }
	            }
	        })

	         $( ".owl-prev").html('<i class="fa fa-chevron-left"></i>');
			 $( ".owl-next").html('<i class="fa fa-chevron-right"></i>');


			 $('.single-circle').click(function(){
			 	$('.stake-modal').css('display', 'flex');
			 })

			 $('.close-x').click(function(){
			 	$('.stake-modal').css('display', 'none');
			 	$('.recorded-modal').css('display', 'none');
			 })

			 $('.s2d').click(function(){
			 	$('.stake-modal').css('display', 'none');
			 	$('.recorded-modal').css('display', 'flex');
			 })

			 // Open side menu on mobile
        	$('.logo-burger').click(function(){
        		$('.sidemenu').css('width', '100%');
        		$('.menu-inside').css('transform', 'translatex(0px)')
        		$('.close-menu').css('display', 'flex')
        	})

        	// close side menu
        	$('.close-menu').click(function(){
        		$('.menu-inside').css('transform', 'translatex(200px)')
        		$('.sidemenu').css('width', '0%');
        		$('.close-menu').css('display', 'none')
        	})
	    </script>
    </body>
</html>